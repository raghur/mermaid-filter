...
const cmd = utils.externalTool('mmdc', process.env)
const imgur = utils.externalTool('imgur', process.env)
let counter = 0
...
function mermaid (type, value, _format, _meta) {
  ...
  const configFile = process.env.MERMAID_FILTER_MERMAID_CONFIG || path.join(folder, '.mermaid-config.json')
  let confFileOpts = ''
  if (fs.existsSync(configFile)) {
    confFileOpts += ` -c "${configFile}"`
  }
  const puppeteerConfig = process.env.MERMAID_FILTER_PUPPETEER_CONFIG || path.join(folder, '.puppeteer.json')
  let puppeteerOpts = ''
  if (fs.existsSync(puppeteerConfig)) {
    puppeteerOpts += ` -p "${puppeteerConfig}"`
  }
  const cssFile = process.env.MERMAID_FILTER_MERMAID_CSS || path.join(folder, '.mermaid.css')
  ...
  const fullCmd = `${cmd}  ${confFileOpts} ${puppeteerOpts} -w ${options.width} -s ${options.scale} -f -i "${tmpfileObj.name}" -t ${options.theme} -b ${options.background} -o "${savePath}"`
